defmodule PortfolioWeb.SkillLive.FormComponent do
  ...
  def update(%{skill: skill} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Resume.change_skill(skill))
     end)}
  end

  @impl true
  def handle_event("validate", %{"skill" => skill_params}, socket) do
    changeset = Resume.change_skill(socket.assigns.skill, skill_params)
  ...
  defp save_skill(socket, :edit, skill_params) do
    case Resume.update_skill(socket.assigns.skill, skill_params) do
      {:ok, skill} ->
  ...
  defp save_skill(socket, :new, skill_params) do
    case Resume.create_skill(skill_params) do
      {:ok, skill} ->
        notify_parent({:saved, skill})
